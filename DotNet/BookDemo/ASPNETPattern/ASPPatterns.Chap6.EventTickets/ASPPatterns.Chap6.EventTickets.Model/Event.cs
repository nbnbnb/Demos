        ...
        public string DetermineWhyATicketCannotbePurchasedWith(Guid reservationId)
        {
            ...
            if (HasReservationWith(reservationId))
            {
                ...
                if (reservation.HasExpired())
                    reservationIssue =
                    String.Format("Ticket reservation ‘{0}’ has expired",
                    reservationId.ToString());
                else if (reservation.HasBeenRedeemed)
                    reservationIssue =
                    String.Format("Ticket reservation ‘{0}’ has already been redeemed",
                    reservationId.ToString());
            }
            else
                reservationIssue =
                String.Format("There is no ticket reservation with the Id ‘{0}’",
                reservationId.ToString());
