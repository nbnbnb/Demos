...
namespace Service
{
    public class WcfHandler : IHttpHandler
    {
        ...
        {
            // 对 HttpRequest 进行解码生成请求消息对象
            Message request = this.MessageEncoderFactory.Encoder
                .ReadMessage(context.Request.InputStream, int.MaxValue, "application/soap+xml; charset=utf-8");

            // 通过请求消息得到代表服务操作的 Action
            string action = request.Headers.Action;

            // 通过 Action 从 MethodInfo 字典中获取服务操作对应的 MethodInfo 对象
            ...
            catch // 虽然会抛出异常，但是对本例来说不会影响程序的执行
            {
            ...
            // 创建服务对象，在 WCF 中服务对象通过 InstanceProvider 创建
            ...
            // 对回复消息进行编码，并将编码后的消息通过 HttpResponse 返回
