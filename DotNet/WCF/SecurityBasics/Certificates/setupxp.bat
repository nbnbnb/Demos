...
for /F "delims=" %%i in ('FindPrivateKey.exe My LocalMachine -n "CN=Woodgrove" -a') do (cacls.exe "%%i" /E /G "%COMPUTERNAME%\ASPNET":R)
for /F "delims=" %%i in ('FindPrivateKey.exe My LocalMachine -n "CN=FabrikamEnterprises" -a') do (cacls.exe "%%i" /E /G "%COMPUTERNAME%\ASPNET":R)
