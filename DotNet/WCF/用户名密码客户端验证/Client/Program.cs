...
namespace Client
{
    class Program
    {
        ...
        {
            using (ChannelFactory<ICalculator> channelFactory = new ChannelFactory<ICalculator>("iisBasic"))
            {
                // Digest 需要在域环境中测试
                ...
                channelFactory.Credentials.UserName.Password = "870615zjx";
            ...
            using (ChannelFactory<ICalculator> channelFactory = new ChannelFactory<ICalculator>("iisWS2007"))
            {
                // Digest 需要在域环境中测试
                // 由于不是 Windows，此处需要显式指定用户名和密码（使用 UserName 属性的 UserName 指定用户名）
